...
# Submit with:
#   sbatch run_ABM_Blood_pipeline_v2.sh /path/to/variants.tsv set_name
# Or run the controller locally:
#   ./run_ABM_Blood_pipeline_v2.sh /path/to/variants.tsv set_name
#
...
VARIANTS=${1:-${VARIANTS:-$ROOT/variant_lists/Vuckovic_and_Chen_BloodCellGWAS.tsv}}
SET=${2:-${SET:-Vuckovic_and_Chen_BloodCellGWAS}}

if [[ ! -r "$VARIANTS" ]]; then
    echo "ERROR: cannot read variant file: $VARIANTS" >&2
    exit 1
fi
if [[ ! "$SET" =~ ^[A-Za-z0-9._-]+$ ]]; then
    echo "ERROR: SET may contain only letters, numbers, dots, underscores, and hyphens" >&2
    exit 1
fi
...
stage1=$SCRIPT_DIR/1_score_variants_ABM_Blood_v2.sh
stage2=$SCRIPT_DIR/2_summarize_variants_ABM_Blood_v2.sh
stage3=$SCRIPT_DIR/3_annotate_variants_ABM_Blood_v2.sh
stage4=$SCRIPT_DIR/4_process_scores_ABM_Blood_v2.sh
...
while true; do
    ...
    if annotations_complete; then
        ...
        if bash "$stage4" "$VARIANTS" "$SET" && [[ -s "$FINAL_OUTPUT" ]]; then
